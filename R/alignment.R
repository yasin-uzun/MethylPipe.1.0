...
plot_alignment_stats <- function(sample_name, df_alignment_stats, coverage_rates)
{
    ...
    alignment_rates = parse_number(df_alignment_stats$Alignment_rate)
    ...
    Alignment.rate = parse_number(df_alignment_stats$Alignment_rate[passing])
...
compute_coverage_rates <- function(alignment_dir)
{
   setwd(alignment_dir)
   ...
   bam_files = list.files(alignment_dir,  pattern = '*.nonCG_filtered.bam$')
   base_counts = c()
   ...
   cell_ids = sub('.rmdup.nonCG_filtered.bam', '', bam_files)
